...
const CRITICAL_WATERMARK_PCT: f32 = 0.90;
const WARNING_WATERMARK_PCT: f32 = 0.75;
const AFRICA_SAFETY_MARGIN: f32 = 0.30;
const DEFAULT_SAFETY_MARGIN: f32 = 0.25;
...
impl MemoryGuard for WatermarkGuard {
    fn can_load_model(&self, model: &ModelMetadata) -> Result<(), MemoryError> {
        ...
        let used_pct = (total - free) as f32 / total as f32;
    ...
    fn request_eviction(&self) -> Result<(), MemoryError> {
        Err(MemoryError::EvictionNotSupported)
    }

    fn start_monitor(&self, interval_ms: u64) {
        info!(interval_ms, "Memory monitor requested (no-op in Phase 1)");
    }
}
...
mod tests {
    ...
    fn test_can_load_small_model() {
        ...
        // Only run this test if system isn't critically loaded
        if used_pct > 0.90 {
            eprintln!(
    ...
    fn test_rejects_huge_model() {
        let guard = WatermarkGuard::new(false, None);
        // A model requiring more RAM than exists should fail
        let total = guard.total_memory_bytes();
